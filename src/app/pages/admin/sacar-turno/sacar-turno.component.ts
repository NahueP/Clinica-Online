...
export class SacarTurnoComponent implements OnInit {
  ...
  constructor(private usuarioSvc : UsuarioFireService, private turnosSvc : TurnosService, private authSvc: AuthService, private alertas: AlertService) 
  {
    ...
    this.pacientes$ = this.usuarioSvc.obtenerTodos('pacientes').valueChanges();
                ...
                listaHorario.forEach(arrayHorario=>{
                  ...
                  for(let i=0;i<arrayHorario.horas.length ; i++)
                  {
                    ...
                    else
                    {
                      ...
                      // this.turnosSvc.obtenerTodosTurnos('turnos').valueChanges().pipe(take(1)).subscribe(obs=>{
                      //   obs.forEach(rsp=>{

                      //     if(!existe)
                      //     {
                      //        if(rsp.especialidad == horario.especialidad && rsp.turno.dia == arrayHorario.dia)
                      //        {
                      //          if(rsp.estado != "Aceptado" && rsp.estado != "Realizado" && rsp.estado != "Pendiente")
                      //          {
                      //             console.log(rsp.especialidad);
                      //             console.log(rsp.turno.dia);
                      //             console.log(rsp.turno.hora);
                      //             this.listaTurnos.push({dia: arrayHorario.dia , hora: arrayHorario.horas[i], fecha: fechaTurno, duracion: arrayHorario.duracion});
